...
def test_visual_registration_rejects_unknown_glyph_level():
    with pytest.raises(ValueError):
        registry.visual("bogus", event="spectrum", glyphs="hieroglyphs")
...
def test_richest_affordable_visual_wins(temporary_slot):
    @registry.visual("meter", event="spectrum", glyphs="ascii")
    class Plain:
        pass

    @registry.visual("meter", event="spectrum", glyphs="braille")
    class Fine:
        pass

    assert registry.pick_visual("meter", "braille").widget is Fine
    assert registry.pick_visual("meter", "blocks").widget is Plain
    assert registry.pick_visual("meter", "ascii").widget is Plain
