...
`HEID//R` целиком стоит на чужой работе, и делать вид, будто это не так, было бы
странно. Ниже — полный список того, у кого и что взято. Документ каждого модуля
повторяет относящиеся к нему строки, чтобы за ними не приходилось идти сюда.

Разница между двумя первыми разделами существенная. «Перенесённый код» означает,
что исходник действительно лежит в этом репозитории и шапка оригинала осталась в
...
| [asciimatics](https://github.com/peterbrittain/asciimatics) | Peter Brittain | Apache-2.0 | Плазменное поле и вращающаяся шестерня, переписанные под здешние отрисовщики |
