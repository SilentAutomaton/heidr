...
падающие глифы были того же цвета, что и логотип над ними. Теперь у каждой
анимации цвет свой, причём предлагается несколько, а один вытягивается при
запуске: сегодня шестерни медные, завтра латунные.
...
программы, `reveal` — ответ, и акцентный цвет принадлежит им. Ни один оттенок в
таблице не подходит к нему близко и не делит с ним индекс xterm — за этим следит
тест.
...
Холст всегда возвращает `rich.text.Text`: у этих трёх он окрашен по уровню, у
остальных — ровно. Цвет выбирается по месту символа в наборе, а соседние символы, попавшие
...
| `runestone` | простой | Два камня по краям экрана, и на них руна за руной высекается строка «Прорицания вёльвы» |
| `etch` | простой | Та же строка, выжженная лазером TerminalTextEffects, если библиотека установлена |
| `yggdrasil` | простой | Ясень, дорастающий до краёв экрана, и руны норн у его корней |
...
Три рунические анимации держатся подальше от середины экрана. Там стоит панель
и закрывает всё, что нарисовано за ней, поэтому камни и выжженный текст стоят по
краям, а ясень растянут так, что ветви достают до краёв, а корни уходят под
панель. На камнях — двадцать вторая строфа «Прорицания вёльвы», «Heiði hana
hétu», записанная младшим футарком: это единственное место, где записано имя,
давшее название программе. `etch` появляется только там, где установлена
[terminaltexteffects](https://github.com/ChrisBuilds/terminaltexteffects), то
есть с дополнением `effects` и в готовой сборке; без неё анимации просто нет
среди вариантов.
...
### Когда долгий шаг молчит

Распознавание речи — самое долгое молчание в программе. Большая модель на окне
в тридцать секунд тратит больше минуты, а съём с трёх остановок — это два таких
окна. Источник в это время уже отработал, события не приходят, а анимация,
которая была на экране, — это картинка данных, которые перестали поступать, и
она замирает.

О том, что всё живо, говорят три вещи. Строка состояния называет шаг и сколько
записи переслушивается. Анимация переключается на шестерни — они означают «работа
идёт, а насколько близок конец, механизм не знает». И вертушка, которая раньше
крутилась только в заголовке окна, теперь крутится и в строке состояния, где её
видно тому, кто смотрит в терминал.

Вертушка включена на весь прогон, а не только на распознавание, так что любой
замолчавший шаг — медленная лента, задумавшаяся модель — показывает тот же знак.
...
Кроме того, выгрузка рисует вокруг терминала окно рабочего стола — с заголовком
и тремя цветными кнопками. Инструмент сдвигает область просмотра на сам
терминал, и на картинках остаётся программа без всякой обвязки. Анимации со
своим циклом — `runestone`, `yggdrasil` и `etch` — записываются ровно на один
цикл: только при такой длине их петля замыкается.

Три ролика — не анимация, а целый прогон: `draw-radio`, `draw-pythia` и
`draw-norns`. Они
...
Логотип вырезан чертами, как вырезают руны: руна состоит из прямых линий,
которые нож проводит поперёк волокон. Буквы белые, а косые — цвета старого льда,
синевато-бирюзовые, и этот цвет не берёт себе ничто в программе, кроме ответа.
Оттенок понижается вслед за терминалом: `#3db4c8` там, где цветов шестнадцать
миллионов, индекс 74 палитры xterm — там, где их 256, и обычный голубой (cyan)
ниже этого. У анимации за логотипом [свой цвет](#цвет), и с
...
`etch` запускает эффект LaserEtch из
[TerminalTextEffects](https://github.com/ChrisBuilds/terminaltexteffects) Криса
Билдса (Chris Builds), MIT, как он есть: библиотека вызывается, код не
копируется. `runestone` берёт у неё только замысел — искру там, где режет
резец, и готовые буквы позади — а высекает сам.

Дерево `yggdrasil` — L-система, которую Аристид Линденмайер опубликовал в
1968 году; её правила никому не принадлежат. Рунам, «Прорицанию вёльвы» и
норнам тысяча лет, и они тоже ничьи. Руны из
[собрания ASCII-рисунков на ascii.co.uk](https://ascii.co.uk/art/runes) работы
Джона Магне Тране (John Magne Trane) и Джона Мелуски (Jon Melusky) мы
разглядывали, но не копировали: лицензии на странице нет, а отсутствие лицензии означает, что все права защищены.
...
Сама Хейд, карты, тарелка, шестерни и рунные камни нарисованы для этого
проекта. Каомодзи
