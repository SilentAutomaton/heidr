...
всякий раз другая: десять модулей вопроса, четырнадцать источников и шесть
толкований дают 840 сочетаний. Семя выбора берётся из измерений, а не из
...
|---|---|---|
| `net` | Работающая сеть; двум источникам нужен `ffmpeg`, ещё по одному — `kiwirecorder.py` и `yt-dlp` | Шесть источников выпадают из выбора |
