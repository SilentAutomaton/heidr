...
Материал извлекается из радиошума, из данных о самолётах, пролетающих
...
Название отсылает к Хейд — вёльве из «Прорицания вёльвы»: её трижды сжигали и трижды
...
толкований дают 1020 сочетаний. Начальное число берётся из измерений, а не из
псевдослучайного генератора: радиошум, публичный маяк случайности, корень
Меркла свежего блока. В коде цепочка называется `Rite`, в документации — прогоном.
...
Есть готовый бинарник, которому Python не нужен, и скрипт, который скачивает его
вместе с внешними программами — приёмником, распознавателем речи, языковой
моделью, — спрашивая по каждой, нужна ли она:
...
Под Windows тот же скрипт называется `install.ps1`: он ставит всё вокруг
программы, а сама программа там пока собирается из исходников — готовый бинарник
есть только для Linux. Про оба скрипта и про сам бинарник написано в отдельном
документе: [бинарники и установщики](docs/ru/binaries.md).
...
Получится `dist/heidr` — исполняемый файл мегабайт на сорок пять, внутри
...
`tools/build_release.sh` делает то же самое в контейнере — именно так собраны
выложенные бинарники. Собери их на роллинг-дистрибутиве, и внутрь попадёт
слишком свежая системная библиотека, с которой они больше нигде не запустятся.
...
мышь работает, `F1` открывает справку, а `ui.splash=false` убирает логотип над
